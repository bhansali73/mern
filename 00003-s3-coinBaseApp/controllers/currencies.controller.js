...
} = require("../validators/currencies.validators"); //Validation for currency queries
...
function getCurrencies(req, res) {
    ...
    const validationErrors = getCurrenciesQueryErrors(req.query);
    ...
    if (filteredData.length > 0) {
        ...
        res.status(404).json({
            message: "No currencies found with the specified min_value",
        });
    }
}
...
function getCurrencyDetails(req, res) {
    ...
    const currencyInfo = coinbaseData.data.find(
